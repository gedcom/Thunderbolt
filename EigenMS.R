...
eig_norm1 = function(m, treatment, prot.info, write_to_file='', progress = NULL){
...
if (!is.null(progress)) progress$set(value = 0, message = "SVD: Selecting complete peptides")
  ...
  for (ii in 1:numiter) {
    if(ii %% 100 == 0) {
      print(ii)
      if (!is.null(progress)) progress$set(0.05*ii/numiter, message = paste0(ii, " / ", numiter))
    }
  ...
  if (!is.null(progress)) progress$set(value = 0.05, message = "Computing SVD, estimating Eigentrends...", detail = "")
...
# tell users how many peptides/metabolites the trends are based on
  ...
  h.c <- sva.id(complete, treatment, n.u.treatment, lm.fm=lm.fm,seed=1234, progress = progress)$n.sv
  print("Number of significant eigenpeptides/trends")
  print(h.c)
  sifplural = if (h.c == 1) "" else "s"
  if (!is.null(progress)) progress$set(0.95, message = paste0(h.c, " significant eigentrend", sifplural,"."), detail = "Plotting...")
  ...
  print("After svd")
  if (!is.null(progress)) progress$set(1, message = "Plot generated.", detail = "")
  par(mfcol=c(3,2))
  par(mar = c(2,2,2,2))
...
sva.id <- function(dat, treatment, n.u.treatment, lm.fm, B=500, sv.sig=0.05, seed=NULL, progress) {
...
if (!is.null(progress)) progress$set(message = "SVD: Starting Bootstrap", detail = "")
# this is the Bootstrap procedure that determines the number of significant eigertrends... 
  for(ii in 1:B){
    if(ii %% 10 == 0) {
      print(paste('Iteration ', ii))
      if (!is.null(progress)) progress$set(0.05 + 0.9 * ii/B, message = "SVD: Bootstrap ", detail = paste("Iteration", ii, "/", B))
    }
